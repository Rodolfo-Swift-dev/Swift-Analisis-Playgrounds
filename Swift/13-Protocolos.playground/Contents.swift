...
//Protocolos
//Error frecuente: asumir que un método definido solo en una extensión del
//protocolo se despacha igual que un requisito declarado por el protocolo.
...
let namedUser = User(name: "Rodolfo", age: 36)
print(namedUser.name)

//creacion de tipo que adopta varios protocolos

protocol Auditable {
  var auditDescription: String { get }
}

struct Student: Named, Identifiable, Auditable {
  let id: UUID
  let name: String
  let course: String
...
let student = Student(id: UUID(), name: "Ana", course: "Swift")
print(student.auditDescription)
...
let teachingAssistant = TeachingAssistant(name: "Camila")
teachingAssistant.study()
teachingAssistant.mentor()
...
protocol DefaultAgeProviding {
  static var defaultAge: Int { get }
}

struct Profile: ProfileProviding, DefaultAgeProviding {
  static let defaultAge = 18
  var displayName: String
  let age: Int
}

var profile = Profile(displayName: "Rodolfo", age: 36)
profile.displayName = "Rodolfo González"
print(profile.displayName, Profile.defaultAge)
...
print(product.name, product.city)
...
let user = User1(
  parameterOne: "Rodolfo",
  parameterTwo: 15)
print(user.parameterOne, user.parameterTwo)
