...
Guía compacta basada en los 20 playgrounds numerados de este proyecto.
...
- **0**
  - **Capítulo:** [Basic](#0-basic)
  - **Conceptos principales:** `let`, `var`, tipos y operadores

- **1**
  - **Capítulo:** [TypeAlias](#1-typealias)
  - **Conceptos principales:** `typealias`

- **2**
  - **Capítulo:** [Tuplas](#2-tuplas)
  - **Conceptos principales:** posiciones, nombres y descomposición

- **3**
  - **Capítulo:** [Optional](#3-optional)
  - **Conceptos principales:** `?`, `nil`, `if let`, `guard let`, `!`, `??`, `?.`

- **4**
  - **Capítulo:** [Colecciones](#4-colecciones)
  - **Conceptos principales:** `Array`, `Set`, `Dictionary`

- **5**
  - **Capítulo:** [ControlFlow](#5-controlflow)
  - **Conceptos principales:** `if`, `guard`, `switch`, `for`, `while`

- **6**
  - **Capítulo:** [Funciones](#6-funciones)
  - **Conceptos principales:** `func`, parámetros, retorno, `inout`

- **7**
  - **Capítulo:** [Clases y estructuras](#7-clases-y-estructuras)
  - **Conceptos principales:** `class`, `struct`, `init`

- **8**
  - **Capítulo:** [Métodos](#8-métodos)
  - **Conceptos principales:** `self`, `mutating`, `static`, `class`

- **9**
  - **Capítulo:** [Propiedades](#9-propiedades)
  - **Conceptos principales:** almacenadas, computadas, observadores y wrappers

- **10**
  - **Capítulo:** [Closures](#10-closures)
  - **Conceptos principales:** `in`, `$0`, trailing, `@escaping`, `@autoclosure`

- **11**
  - **Capítulo:** [Enum](#11-enum)
  - **Conceptos principales:** `enum`, `case`, valores asociados y raw values

- **12**
  - **Capítulo:** [Herencia](#12-herencia)
  - **Conceptos principales:** `super`, `override`, `final`

- **13**
  - **Capítulo:** [Protocolos](#13-protocolos)
  - **Conceptos principales:** `protocol`, conformidad, requisitos y extensiones

- **14**
  - **Capítulo:** [Genéricos](#14-genéricos)
  - **Conceptos principales:** `<T>`, `where`, `associatedtype`

- **15**
  - **Capítulo:** [Extensiones](#15-extensiones)
  - **Conceptos principales:** `extension`

- **16**
  - **Capítulo:** [Access Level](#16-access-level)
  - **Conceptos principales:** `private`, `fileprivate`, `internal`, `public`, `open`

- **17**
  - **Capítulo:** [Gestión de errores](#17-gestión-de-errores)
  - **Conceptos principales:** `Error`, `throws`, `throw`, `try`, `do`, `catch`

- **18**
  - **Capítulo:** [Type Casting](#18-type-casting)
  - **Conceptos principales:** `is`, `as`, `as?`, `as!`

- **19**
  - **Capítulo:** [Operador ternario](#19-operador-ternario)
  - **Conceptos principales:** `condición ? A : B`
...
- **`self`**
  ...
  - **Límite o regla:** Suele ser opcional, salvo para desambiguar nombres.
...
## Mapa final de palabras clave

- **Datos básicos:** `let`, `var`, `Int`, `Float`, `Double`, `String`, `Bool`

- **Alias y agrupación:** `typealias`, tuplas

- **Ausencia de valor:** `?`, `nil`, `if let`, `guard let`, `!`, `??`, `?.`

- **Colecciones:** `Array`, `Set`, `Dictionary`, `Hashable`

- **Flujo:** `if`, `else`, `guard`, `switch`, `case`, `where`, `for`, `while`, `repeat`

- **Funciones:** `func`, `return`, `_`, `...`, `inout`, `&`

- **Tipos:** `struct`, `class`, `enum`, `protocol`, `extension`

- **Métodos y propiedades:** `self`, `mutating`, `static`, `class`, `get`, `set`, `willSet`, `didSet`

- **Wrappers:** `@propertyWrapper`, `wrappedValue`

- **Closures:** `in`, `$0`, `@escaping`, `@autoclosure`

- **Herencia:** `super`, `override`, `final`

- **Genéricos:** `<T>`, `where`, `associatedtype`

- **Acceso:** `private`, `fileprivate`, `internal`, `package`, `public`, `open`

- **Errores:** `Error`, `throws`, `throws(ErrorType)`, `throw`, `try`, `do`, `catch`, `try?`, `try!`, `defer`

- **Type casting:** `is`, `as`, `as?`, `as!`

- **Decisión breve:** `condición ? A : B`


Método de estudio recomendado: para cada fila, predecir el resultado, ejecutar el ejemplo del playground y luego provocar deliberadamente el límite descrito.
